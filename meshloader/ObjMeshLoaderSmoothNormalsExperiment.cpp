//
//  ObjMeshLoaderSmoothNormalsExperiment.cpp
//  OpenGLOtherTest
//
...
bool ObjMeshLoaderSmoothNormalsExperiment::isSplitVertex( const GLuint & index, const GLuint & normalIndex, std::map<GLuint,std::vector<GLuint>*> * normalIndexMap ){
    return false;
}

void ObjMeshLoaderSmoothNormalsExperiment::loadFromFile( const GLchar * path )
{
    std::string line;
    ...
    while ( std::getline( in, line ) )
    {
        ...
        {
            pos = line.find(" ", pos);
            //printf("found s . position: %zd", pos);
            if ( line.find_first_of("0", pos) >= line.length() && line.find_first_of("off", pos) >= line.length() )
            {
                //printf("did not find 0 or off.");
                isSmoothingGroup = true;
            }
            else
                isSmoothingGroup = false;
            
            //std::cout << "isSmoothingGroup " << isSmoothingGroup << std::endl;
        }
